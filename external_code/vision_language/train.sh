...
for model in ViT-B-32;do
	...
	for ratio in 1;do
	CUDA_VISIBLE_DEVICES="2" accelerate launch src/finetune.py --model=$model --seed 1 --fname results/${model}_RandLoRA6 --rand-lora --rank 6 --data-ratio $ratio --param-type 'randlora' --full-clip
	# CUDA_VISIBLE_DEVICES="2" python src/finetune.py --model=$model --seed 1 --fname results/${model}_RandLoRA6 --rand-lora --rank 6 --data-ratio $ratio --param-type 'randlora' --full-clip
